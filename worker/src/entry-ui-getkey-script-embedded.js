...
function scriptMatch(pathname) {
  return pathname.match(/^\/api\/v1\/scripts\/([^/]+)\/embedded-loader$/);
}

async function findScriptForOwner(env, scriptId, ownerId) {
  return env.DB.prepare(`SELECT s.id, s.service_id, s.status
    FROM scripts s
    JOIN frezen_key_services sv ON sv.id = s.service_id
    WHERE s.id = ?1 AND sv.owner_id = ?2
    LIMIT 1`).bind(scriptId, ownerId).first();
}

async function chooseKeyRecord(env, scriptId, ownerId, keyRecordId = '') {
  if (keyRecordId) {
    return env.DB.prepare(`SELECT k.id, k.license_id, l.status AS license_status, l.expires_at
      FROM frezen_key_records k
      JOIN licenses l ON l.id = k.license_id
      JOIN scripts s ON s.service_id = k.service_id
      JOIN frezen_key_services sv ON sv.id = s.service_id
      WHERE k.id = ?1 AND s.id = ?2 AND sv.owner_id = ?3
      LIMIT 1`).bind(keyRecordId, scriptId, ownerId).first();
  }
  return env.DB.prepare(`SELECT k.id, k.license_id, l.status AS license_status, l.expires_at
    FROM frezen_key_records k
    JOIN licenses l ON l.id = k.license_id
...
async function generateEmbeddedLoader(request, env, requestId, scriptId) {
  ...
  const script = await findScriptForOwner(env, scriptId, access.user_id);
  if (!script) return json({ error: 'SCRIPT_NOT_FOUND', request_id: requestId }, 404, requestId);
  ...
  return json({
    loader_type: 'embedded',
    enabled: true,
    script_id: scriptId,
    key_record_id: record.id,
    license_id: record.license_id,
    source,
    note: 'This loader embeds a signed delivery credential instead of the plaintext license key. The existing HWID/license validation remains active.',
...
async function deliverWithEmbeddedToken(request, env, requestId, scriptId) {
  ...
  const token = url.searchParams.get('delivery_token')?.trim() ?? '';
  if (!token) return null;
  if (!isEnabled(env, 'FREZEN_EMBEDDED_LOADER_ENABLED')) return text('EMBEDDED_LOADER_DISABLED', 403, requestId);

  const payload = await verifyEmbeddedDeliveryToken(env, token);
  if (!payload || payload.scriptId !== scriptId) return text('INVALID_DELIVERY_TOKEN', 403, requestId);
  if (!env.DB) return text('SCRIPT_DELIVERY_UNAVAILABLE', 503, requestId);

  try {
    const row = await env.DB.prepare(`SELECT k.id, k.service_id, k.owner_id, k.license_id, k.key_secret_ciphertext,
        l.status AS license_status, l.expires_at, s.service_id AS script_service_id, s.status AS script_status
      FROM frezen_key_records k
      JOIN licenses l ON l.id = k.license_id
      JOIN scripts s ON s.id = ?1
      WHERE k.id = ?2
      LIMIT 1`).bind(scriptId, payload.keyRecordId).first();
    ...
    forwarded.searchParams.delete('delivery_token');
    forwarded.searchParams.set('key', licenseKey);
    const proxiedRequest = new Request(forwarded.toString(), request);
    const response = await deliverScriptFileByKey(proxiedRequest, env, requestId, scriptId);
    const headers = new Headers(response.headers);
    headers.set('x-frezen-loader-mode', 'embedded');
    headers.set('x-frezen-delivery-token', 'verified');
    return new Response(response.body, { status: response.status, statusText: response.statusText, headers });
  } catch (error) {
    console.error('embedded loader delivery failed', { requestId, scriptId, message: String(error?.message || error) });
    return text('SCRIPT_DELIVERY_UNAVAILABLE', 503, requestId);
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (loaderApiMatch) return generateEmbeddedLoader(request, env, crypto.randomUUID(), decodeURIComponent(loaderApiMatch[1]));

    const fileMatch = url.pathname.match(/^\/files\/([^/]+)\.lua\/?$/i);
    if (fileMatch && url.searchParams.has('delivery_token')) {
      return deliverWithEmbeddedToken(request, env, crypto.randomUUID(), decodeURIComponent(fileMatch[1]));
    }

    if (!isEnabled(env, 'FREZEN_KEY_LOADER_ENABLED')) {
      const loaderMatch = url.pathname.match(/^\/(?:loader|compact-loader)\/([^/]+)\/?$/);
      if (loaderMatch || url.pathname.startsWith('/loader/')) return text('KEY_LOADER_DISABLED', 403, crypto.randomUUID());
      if (url.pathname.match(/^\/files\/([^/]+)\.lua\/?$/i) && url.searchParams.has('key')) return text('KEY_LOADER_DISABLED', 403, crypto.randomUUID());
    }
