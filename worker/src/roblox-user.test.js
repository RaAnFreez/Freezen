...
describe("Roblox username resolver", () => {
  ...
  it("resolves a numeric Roblox user id", async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith("https://users.roblox.com/v1/users/123456", { headers: { accept: "application/json" } });
  });
  ...
  it("hydrates missing usernames and persists the resolved value", async () => {
    ...
    const updates = [];
    const env = { DB: { prepare(sql) { return { bind(...params) { return { async run() { updates.push({ sql, params }); } }; } }; } } };
    const devices = [
  ...
  it("falls back to displayName when name is unavailable", async () => {
