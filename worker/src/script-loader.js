...
export async function deliverScriptByKey(request, env, requestId, scriptId) {
  ...
  if (/text\/html/i.test(request.headers.get('accept') ?? '')) return deny();
  ...
  try {
    ...
    const row = await env.DB.prepare(`
      SELECT s.id AS script_id, s.status AS script_status,
             v.version, v.status AS version_status,
             f.content, f.content_type
      FROM scripts s
      JOIN script_versions v ON v.script_id = s.id AND v.status = 'ACTIVE'
      JOIN script_files f ON f.script_version_id = v.id
      JOIN frezen_key_records kr ON kr.service_id = s.service_id
