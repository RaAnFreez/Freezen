...
const slugify = (value) => String(value || '').trim().toLowerCase().replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 50);
const parseArray = (value) => { try { const parsed = JSON.parse(value); return Array.isArray(parsed) ? parsed : []; } catch { return []; } };
const parseJsonObject = (value) => { try { const parsed = JSON.parse(value || 'null'); return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : null; } catch { return null; } };
...
export async function getCanonicalDashboardState(env, access) {
  if (!env?.DB || !access?.user_id) return json({ error: 'DATABASE_UNAVAILABLE' }, 503);
  try {
    const [services, providers, checkpoints] = await Promise.all([
      env.DB.prepare(`SELECT id,name,slug,description,premium,keyless,keyless_days_json,active,created_at,updated_at FROM frezen_key_services WHERE owner_id=?1 AND active=1 ORDER BY name COLLATE NOCASE`).bind(access.user_id).all(),
      env.DB.prepare(`SELECT id,name,type,service_id,active,checkpoints_json,settings_json,created_at,updated_at FROM frezen_key_providers WHERE owner_id=?1 AND active=1 ORDER BY name COLLATE NOCASE`).bind(access.user_id).all(),
      env.DB.prepare(`SELECT id,name,type,url,active,metadata_json,created_at,updated_at FROM frezen_key_checkpoints WHERE owner_id=?1 AND active=1 ORDER BY name COLLATE NOCASE`).bind(access.user_id).all(),
    ]);

    const serviceRows = (services.results || []).map((row) => ({ ...row, premium: Boolean(row.premium), keyless: Boolean(row.keyless), days: parseArray(row.keyless_days_json) }));
    const providerRows = (providers.results || []).map((row) => {
      const settings = parseJsonObject(row.settings_json) || {};
      const { settings_json: _settings, checkpoints_json: _checkpoints, ...base } = row;
      return { ...settings, ...base, active: row.active !== 0, checkpoints: checkpointRefs(row.checkpoints_json) };
    });
    const checkpointRows = (checkpoints.results || []).map((row) => {
      const metadata = parseJsonObject(row.metadata_json) || {};
      const { metadata_json: _metadata, ...base } = row;
      return { ...metadata, ...base, active: row.active !== 0 };
    });
    return json({ canonical: true, services: serviceRows, providers: providerRows, checkpoints: checkpointRows });
  } catch (error) {
    console.error('canonical dashboard state read failed', { owner_id: access.user_id, message: String(error?.message || error) });
...
export async function reconcileDashboardState(request, env, access) {
  ...
  try {
    ...
    if (missingServiceIds.length) {
      ...
      await env.DB.prepare(`UPDATE frezen_key_providers SET active=0,updated_at=CURRENT_TIMESTAMP WHERE owner_id=?1 AND service_id IN (${missingServiceIds.map(()=>'?').join(',')})`).bind(ownerId,...missingServiceIds).run();
      await env.DB.prepare(`UPDATE scripts SET status='DISABLED',updated_at=CURRENT_TIMESTAMP WHERE service_id IN (${missingServiceIds.map(()=>'?').join(',')})`).bind(...missingServiceIds).run();
    }

    const missingProviders = await env.DB.prepare(`SELECT id FROM frezen_key_providers WHERE owner_id=?1 AND active=1${providerIds.size?` AND id NOT IN (${[...providerIds].map(()=>'?').join(',')})`:''}`).bind(ownerId,...providerIds).all();
    const missingProviderIds = (missingProviders?.results||[]).map((row)=>row.id).filter(Boolean);
    if (missingProviderIds.length) await env.DB.prepare(`UPDATE frezen_key_providers SET active=0,updated_at=CURRENT_TIMESTAMP WHERE owner_id=?1 AND id IN (${missingProviderIds.map(()=>'?').join(',')})`).bind(ownerId,...missingProviderIds).run();

    const missingCheckpoints = await env.DB.prepare(`SELECT id FROM frezen_key_checkpoints WHERE owner_id=?1 AND active=1${checkpointIds.size?` AND id NOT IN (${[...checkpointIds].map(()=>'?').join(',')})`:''}`).bind(ownerId,...checkpointIds).all();
    const missingCheckpointIds = (missingCheckpoints?.results||[]).map((row)=>row.id).filter(Boolean);
    if (missingCheckpointIds.length) await env.DB.prepare(`UPDATE frezen_key_checkpoints SET active=0,updated_at=CURRENT_TIMESTAMP WHERE owner_id=?1 AND id IN (${missingCheckpointIds.map(()=>'?').join(',')})`).bind(ownerId,...missingCheckpointIds).run();
