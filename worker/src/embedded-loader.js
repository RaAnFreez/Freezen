...
export function buildEmbeddedLoaderSource(request, scriptId, deliveryToken) {
  ...
  return [
    ...
    'local __frezen_http_ok,__frezen_source=false,nil;',
    'local __frezen_last_error=nil;',
    'for __frezen_attempt=1,4 do',
    '  __frezen_http_ok,__frezen_source=pcall(function() return game:HttpGet(__frezen_url) end);',
    '  if __frezen_http_ok and type(__frezen_source)=="string" and __frezen_source~="" then break end;',
    '  __frezen_last_error=__frezen_source;',
    '  local __frezen_wait=(task and task.wait) or wait;',
    '  if __frezen_attempt<4 and type(__frezen_wait)=="function" then pcall(__frezen_wait,0.5*__frezen_attempt) end;',
    'end;',
    'if not __frezen_http_ok then error("FREZEN_PAYLOAD_HTTP_FAILED:"..tostring(__frezen_last_error)) end;',
    'if type(__frezen_source)~="string" or __frezen_source=="" then error("FREZEN_PAYLOAD_EMPTY") end;',
    'local __frezen_load=loadstring or load;',
    'if type(__frezen_load)~="function" then error("FREZEN_LOADSTRING_UNAVAILABLE") end;',
    'local __frezen_chunk,__frezen_compile_error=__frezen_load(__frezen_source);',
