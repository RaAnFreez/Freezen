...
async function ensureSchema(env) {
  ...
  try {
    await env.DB.prepare(`CREATE TABLE IF NOT EXISTS hwid_bindings_v2 (
      id TEXT PRIMARY KEY,
      owner_id TEXT,
    ...
    await env.DB.batch([
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_hwid_v2_owner ON hwid_bindings_v2(owner_id, status)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_hwid_v2_license ON hwid_bindings_v2(license_id, status)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_hwid_v2_last_seen ON hwid_bindings_v2(last_seen)"),
    ]);
    return true;
  } catch (error) {
    console.error("HWID V2 schema initialization failed", { message: String(error?.message ?? error) });
    return false;
  }
}
...
async function licenseFor(env, licenseId) {
  try {
    return await env.DB.prepare("SELECT id, user_id, status, expires_at FROM licenses WHERE id = ?1 LIMIT 1").bind(licenseId).first();
  } catch {
    return null;
  }
}
...
export async function bindHwidV2(env, { licenseId, ownerId = null, rawHwid }) {
  ...
  if (!(await ensureSchema(env))) return { ok: false, reason: "DATABASE_UNAVAILABLE" };
...
export async function listHwidV2(env, { ownerId, licenseId = null }) {
  ...
  try {
    ...
    const rows = await env.DB.prepare(`SELECT h.id, h.license_id, h.status, h.first_seen, h.last_seen, h.blocked_at, h.blocked_reason, h.created_at, h.updated_at, l.expires_at, substr(h.hwid_hash, 1, 12) AS fingerprint FROM hwid_bindings_v2 h JOIN licenses l ON l.id = h.license_id ${where} ORDER BY CASE WHEN h.status = 'blocked' THEN 0 ELSE 1 END, h.last_seen DESC LIMIT 500`).bind(...bindings).all();
    const devices = rows.results ?? [];
    for (const device of devices) {
      try {
        const key = await env.DB.prepare("SELECT key_name, service_id FROM frezen_key_records WHERE license_id = ?1 ORDER BY created_at DESC LIMIT 1").bind(device.license_id).first();
        device.key_name = key?.key_name ?? null;
        device.service_id = key?.service_id ?? null;
      } catch {
        device.key_name = null;
        device.service_id = null;
      }
    }
...
export async function cleanupHwidV2(env) {
  if (!env?.DB || !(await ensureSchema(env))) return { removed: 0 };
  try {
