...
export async function bindHwidV2(env, { licenseId, ownerId = null, rawHwid }) {
  ...
  try {
    ...
    const ownerBlocked = await getOwnerBlockedBinding(env, owner, hwidHash);
    if (ownerBlocked) return { ok: false, reason: "HWID_BLOCKED" };
...
export async function validateHwidV2(env, { licenseId, rawHwid, ownerId = null }) {
  ...
  try {
    ...
    const hwidHash = await sha256Hex(hwid);
    const blocked = await getOwnerBlockedBinding(env, ownerId ?? license.user_id ?? null, hwidHash);
    if (blocked) return { ok: false, reason: "HWID_BLOCKED" };
