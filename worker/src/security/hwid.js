...
const readJson = async (request, requestId) => {
  ...
  catch { return new Response(JSON.stringify({ error: "INVALID_JSON", request_id: requestId }), { status: 400, headers: { "content-type": "application/json; charset=utf-8" } }); }
};
...
export async function listHwid(request, env, requestId, _json) {
  const licenseId = new URL(request.url).searchParams.get("license_id") || "";
  const result = await listHwidV2(env, { ownerId: request?.auth?.user_id ?? null, licenseId });
  if (!result.ok) return json({ error: result.reason }, statusCode(result.reason, 401), requestId);
...
export async function resetHwid(request, env, requestId, _json, licenseId, auth = null) {
  if (!auth?.user_id) return json({ error: "SESSION_AUTH_REQUIRED" }, 401, requestId);
  const result = await resetHwidV2(env, { ownerId: auth.user_id, licenseId });
...
export async function blockHwid(request, env, requestId, _json, deviceId, auth = null) {
  if (!auth?.user_id) return json({ error: "SESSION_AUTH_REQUIRED" }, 401, requestId);
  const result = await setHwidStatusV2(env, { ownerId: auth.user_id, deviceId, status: "blocked" });
  if (!result.ok) return json({ error: result.reason }, statusCode(result.reason, 400), requestId);
...
export async function unblockHwid(request, env, requestId, _json, deviceId, auth = null) {
  if (!auth?.user_id) return json({ error: "SESSION_AUTH_REQUIRED" }, 401, requestId);
  const result = await setHwidStatusV2(env, { ownerId: auth.user_id, deviceId, status: "active" });
  if (!result.ok) return json({ error: result.reason }, statusCode(result.reason, 400), requestId);
