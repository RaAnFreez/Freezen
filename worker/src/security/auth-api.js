...
async function isLoginRateLimited(db, email) {
  const row = await db.prepare(
    "SELECT window_started_at, attempts FROM auth_rate_limits WHERE identifier = ?1 LIMIT 1",
  ).bind(rateLimitKey(email)).first();
  if (!row) return false;
...
async function recordLoginFailure(db, email) {
  ...
  if (!row || Date.now() - Date.parse(row.window_started_at) >= LOGIN_WINDOW_MS) {
    await db.prepare(
      "INSERT OR REPLACE INTO auth_rate_limits (identifier, window_started_at, attempts, updated_at) VALUES (?1, ?2, 1, ?2)",
    ).bind(key, now).run();
    return;
  }
  await db.prepare(
    "UPDATE auth_rate_limits SET attempts = attempts + 1, updated_at = ?2 WHERE identifier = ?1",
  ).bind(key, now).run();
}
...
export async function login(request, env, requestId, json) {
  ...
  try {
    ...
    const user = await env.DB.prepare(
      "SELECT id, email, username, password_hash, role, status FROM users WHERE email = ?1 LIMIT 1",
    ).bind(email).first();
    ...
    // Rotate/revoke existing sessions for this login to limit session fixation.
...
export async function logout(request, env, requestId, json) {
  ...
  try {
    ...
    return json({ authenticated: false, request_id: requestId }, 200, requestId, { "set-cookie": clearSessionCookie() });
  } catch {
...
export async function listSessions(request, env, requestId, json, currentUser) {
  try {
    const result = await env.DB.prepare(
      "SELECT id, created_at, expires_at, revoked_at, last_seen_at FROM sessions WHERE user_id = ?1 ORDER BY created_at DESC LIMIT 50",
...
export async function revokeSession(request, env, requestId, json, currentUser, sessionId) {
  ...
  try {
    ...
    ).bind(sessionId, currentUser.user_id).run();
...
export async function forgotPassword(request, env, requestId, json) {
  ...
  try {
    ...
    // A delivery provider is intentionally not guessed. Development/test exposes the one-time token only
    // so the flow can be verified without an email provider. Production never exposes it in the API response.
    if (env.FREZEN_ENV !== "production") response.reset_token = reset.token;
    return json(response, 200, requestId);
  } catch {
    return json({ error: "PASSWORD_RESET_ERROR", request_id: requestId }, 503, requestId);
  }
}
...
export async function resetPassword(request, env, requestId, json) {
  ...
  const password = body?.password;
  if (!token || typeof password !== "string" || password.length < MIN_PASSWORD_LENGTH || password.length > 256) {
    return json({ error: "INVALID_RESET_REQUEST", request_id: requestId }, 400, requestId);
  }
  ...
  try {
    ...
    return json({ password_reset: true, message: "Password reset successfully. Please log in again.", request_id: requestId }, 200, requestId);
  } catch {
    return json({ error: "PASSWORD_RESET_ERROR", request_id: requestId }, 503, requestId);
  }
}
