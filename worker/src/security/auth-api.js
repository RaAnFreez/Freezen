...
export async function login(request, env, requestId, json) {
  ...
  try {
    if (await isLoginRateLimited(env.DB, email)) return json({ error: "RATE_LIMITED", message: "Too many login attempts. Try again later.", request_id: requestId }, 429, requestId);
    ...
    if (!user || !validPassword || user.status !== "ACTIVE") { await recordLoginFailure(env.DB, email); return json({ error: "INVALID_CREDENTIALS", request_id: requestId }, 401, requestId); }
...
export async function revokeSession(request, env, requestId, json, currentUser, sessionId) { if (!sessionId || sessionId.length > 128) return json({ error: "INVALID_SESSION_ID", request_id: requestId }, 400, requestId); try { const result = await env.DB.prepare("UPDATE sessions SET revoked_at = CURRENT_TIMESTAMP WHERE id = ?1 AND user_id = ?2 AND revoked_at IS NULL").bind(sessionId, currentUser.user_id).run(); if (!result?.meta?.changes) return json({ error: "SESSION_NOT_FOUND", request_id: requestId }, 404, requestId); return json({ revoked: true, request_id: requestId }, 200, requestId); } catch { return json({ error: "DATABASE_ERROR", request_id: requestId }, 503, requestId); } }
