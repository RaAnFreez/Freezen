(() => {
  ...
  function loaderSource(scriptId) {
    ...
    const bootstrap = `${location.origin}/loader/${id}?bootstrap=1&key=`;
    return [
      'script_key="PASTE YOUR KEY HERE";',
      `loadstring(game:HttpGet("${bootstrap}"..game:GetService("HttpService"):UrlEncode(script_key)))()`,
    ].join('\n');
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    navigator.clipboard?.writeText(source)
      .then(() => alert('Frezen compact loader copied. Replace PASTE YOUR KEY HERE with a valid key. Runtime HWID capture remains enabled.'))
      .catch(() => alert(source));
  }, true);
