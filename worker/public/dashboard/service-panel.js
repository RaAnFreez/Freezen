...
function modal({ edit = false, service = null }) {
  ...
  wrap.innerHTML = `<section class="service-modal" role="dialog" aria-modal="true"><div class="service-modal-head"><div><div class="service-icon">▦</div><h3>${edit ? 'Edit Service' : 'New Service'}</h3><p>${edit ? 'Modify service settings' : 'Create and configure a new service'}</p></div><button class="service-close" type="button">×</button></div><form class="service-form"><label>Name <span>*</span><input id="service-name" maxlength="100" required value="${esc(service?.name || '')}" placeholder="Frezen Gui"></label><label>Description<input id="service-description" maxlength="500" value="${esc(service?.description || '')}" placeholder="Optional service description"></label><label>Configured Link<input id="service-link" maxlength="160" value="${esc(slug ? `${origin}/get-key/${slug}` : '')}" placeholder="https://example.com/get-key/frezen"><small>Public key link for this service. The link is configured here, not in Provider.</small></label><label>Service Slug<input id="service-slug" maxlength="80" pattern="[A-Za-z0-9_-]+" value="${esc(slug)}" placeholder="frezen"><small>Example: <b>/get-key/frezen</b></small></label><div class="service-option"><div><b>Premium Service</b><small>Mark this service as premium.</small></div><input id="service-premium" type="checkbox" ${service?.premium ? 'checked' : ''}></div><div class="service-option"><div><b>Keyless Mode</b><small>Skip key verification for this service.</small></div><input id="service-keyless" type="checkbox" ${service?.keyless ? 'checked' : ''}></div><div class="service-days"><b>Keyless weekdays</b><small>Automatically keyless on selected weekdays (UTC).</small><div class="day-grid">${days.map((d,i)=>`<label><span>${d}</span><input class="service-day" type="checkbox" data-day="${i}" ${service?.days?.includes(i) ? 'checked' : ''}></label>`).join('')}</div></div><p class="service-note">Configured Link belongs to the Service. Provider configuration will remain separate and can be added later.</p><div class="service-form-actions"><button class="primary" type="submit">${edit ? 'Save' : 'Create'}</button><button class="secondary" type="button" id="service-cancel">Cancel</button></div></form></section>`;
  ...
  wrap.querySelector('form').onsubmit = (e) => {
    ...
    const serviceSlug = wrap.querySelector('#service-slug').value.trim().replace(/[^A-Za-z0-9_-]/g, '');
    if (!name) return;
    const configuredLink = serviceSlug ? `${origin}/get-key/${serviceSlug}` : '';
    const item = { id: service?.id || id(), name, description: wrap.querySelector('#service-description').value.trim(), slug: serviceSlug, configured_link: configuredLink, premium: wrap.querySelector('#service-premium').checked, keyless: wrap.querySelector('#service-keyless').checked, days: [...wrap.querySelectorAll('.service-day:checked')].map(x => Number(x.dataset.day)), updated_at: new Date().toISOString() };
    const list = read();
