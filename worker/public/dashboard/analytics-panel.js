...
async function loadAnalytics(range = '30d') {
  ...
  try {
    const response = await fetch(`/api/v1/dashboard/overview?range=${encodeURIComponent(range)}`, {
    ...
    renderAnalytics(content, data, range);
  } catch (error) {
...
function metric(label, value, note, glyph) {
  return `<article class="stat"><div class="stat-icon">${glyph}</div><p>${esc(label)}</p><strong>${num(value)}</strong><small>${esc(note)}</small></article>`;
}
...
function renderAnalytics(content, data, range) {
  ...
  const points = [...licenseSeries, ...scriptSeries].map((x) => Number(x.count || 0));
  ...
  content.innerHTML = `
    <div class="hero">
      <div><p class="eyebrow">PHASE 12 · ANALYTICS</p><h2>Frezen Analytics Center</h2><p>Authenticated operational metrics from the existing dashboard API. No fabricated values are used.</p></div>
      <span class="badge"><span class="dot"></span>Live API</span>
    </div>
    <div class="tabs" style="margin:16px 0">${['24h','7d','30d','90d'].map((r) => `<button class="${r === range ? 'active' : ''}" data-analytics-range="${r}">${r.toUpperCase()}</button>`).join('')}</div>
    <div class="stats">
      ${metric('Total Licenses', m.total_licenses, 'All license records', '◇')}
      ${metric('Active Licenses', m.active_licenses, 'Currently valid', '✓')}
      ${metric('Users', m.users, 'Active accounts', '♙')}
      ${metric('Script Requests', m.script_requests, `Within ${range}`, '{}')}
      ${metric('SafeLinkU Claims', m.safelinku_claims, `Successful · ${range}`, '↗')}
      ${metric('HWID Resets', m.hwid_resets, `Within ${range}`, '⌘')}
      ${metric('Expired', m.expired_licenses, 'Past expiration', '◷')}
      ${metric('Revoked', m.revoked_licenses, 'Revoked licenses', '⊘')}
    </div>
    <div class="columns">
      <section class="panel"><div class="panel-head"><div><p class="eyebrow">LICENSE ACTIVITY</p><h3>Daily activity</h3></div><span class="eyebrow">${esc(range)}</span></div><div class="chart">${bars}</div><p class="chart-note">Bars represent authenticated license creation activity returned by the production overview endpoint.</p></section>
      <section class="panel"><div class="panel-head"><div><p class="eyebrow">SCRIPT DELIVERY</p><h3>Request activity</h3></div></div>${scriptSeries.length ? scriptSeries.slice(-8).map((x) => `<div class="activity"><span>${esc(x.date)}<small>Authorized script requests</small></span><b>${num(x.count)}</b></div>`).join('') : '<div class="empty"><b>No script requests</b><span>No script request events were recorded for this range.</span></div>'}</section>
    </div>
    <section class="panel recent-panel"><div class="panel-head"><div><p class="eyebrow">AUDIT STREAM</p><h3>Recent activity</h3></div><button class="ghost" data-section="audit">View logs</button></div>${recent}</section>
