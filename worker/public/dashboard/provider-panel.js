...
function openProviderModal({ edit = false, provider = null }) {
  ...
  wrap.innerHTML = `<section class="provider-modal" role="dialog" aria-modal="true"><div class="provider-modal-head"><div><div class="provider-icon">⌁</div><h3>${edit ? 'Edit Provider' : 'New Provider'}</h3><p>${edit ? 'Modify key-system provider settings' : 'Create a provider for an existing service'}</p></div><button class="provider-close" type="button">×</button></div><form class="provider-form"><div class="provider-field"><label>Provider name <span class="provider-required">*</span></label><input id="provider-name" maxlength="100" required value="${esc(provider?.name || '')}" placeholder="SafeLinkU Provider"></div><div class="provider-field"><label>Service <span class="provider-required">*</span></label><select id="provider-service" required><option value="">Select a service</option>${services.map(s => `<option value="${esc(s.id)}" ${provider?.service_id === s.id ? 'selected' : ''}>${esc(s.name)}</option>`).join('')}</select>${!services.length ? '<small class="provider-help">Create a Service first, then connect it here.</small>' : ''}</div><div class="provider-field"><label>Provider type</label><select id="provider-type"><option value="safelinku" selected>SafeLinkU</option></select></div><div class="provider-connection ${safeMeta?.key_configured ? 'ready' : 'pending'}"><span class="provider-connection-dot"></span><div><b>SafeLinkU integration</b><small>${safeMeta?.key_configured ? 'Configured and available to this provider.' : 'Pending — configure SafeLinkU in its own tab first.'}</small></div></div><p class="provider-note">The configured public key link belongs to the Service. Provider only connects the selected service to the provider type.</p><div class="provider-actions-row"><button class="primary" type="submit" ${!services.length ? 'disabled' : ''}>${edit ? 'Save' : 'Create'}</button><button class="secondary" type="button" id="provider-cancel">Cancel</button></div></form></section>`;
  ...
  wrap.querySelector('form').onsubmit = (e) => {
    ...
    const item = { id: provider?.id || makeId(), name, service_id: serviceId, type: 'safelinku', active: true, updated_at: new Date().toISOString() };
