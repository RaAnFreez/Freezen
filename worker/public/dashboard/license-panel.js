(() => {
  ...
  function showMessage(text, kind = 'ok') { const node = document.querySelector('#key-message'); if (node) node.innerHTML = text ? `<div class="key-message ${kind}">${esc(text)}</div>` : ''; }
  ...
  async function loadKeys() {
    ...
    try {
      ...
      if (!keys.length) list.innerHTML = `<div class="key-empty"><div class="empty-icon">⌁</div><strong>No keys available yet.</strong><div>Create your first key to start managing access.</div></div>`;
      else {
  ...
  function openCreateModal() {
    ...
    modal.querySelectorAll('.key-preset').forEach((button) => button.onclick = () => {
      ...
      const map = { '1h':[0,1,0], '24h':[1,0,0], '7d':[7,0,0], '30d':[30,0,0], '90d':[90,0,0] }[button.dataset.preset];
      days.value = map[0]; hours.value = map[1]; minutes.value = map[2]; forever.checked = false; syncValidity();
    });
    ...
    modal.querySelector('#create-key-form').onsubmit = async (event) => {
      event.preventDefault(); msg.hidden = true; result.innerHTML = '';
      const form = new FormData(event.currentTarget);
      const payload = { provider_id: form.get('provider_id'), service_id: form.get('service_id') || null, folder_id: form.get('folder_id') || null, key_name: form.get('key_name') || null, days: Number(form.get('days') || 0), hours: Number(form.get('hours') || 0), minutes: Number(form.get('minutes') || 0), forever: forever.checked, premium: modal.querySelector('#key-premium').checked, max_devices: Number(form.get('max_devices') || 1) };
      const button = modal.querySelector('#key-generate'); button.disabled = true;
      try {
        ...
        await loadKeys();
      } catch (error) { msg.hidden = false; msg.className = 'key-message error'; msg.textContent = error.message; } finally { button.disabled = false; }
    };
  }
