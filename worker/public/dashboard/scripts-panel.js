...
let scriptItems = [];
let scriptSystemAvailable = false;
...
async function checkScriptSystem() {
  try {
    const data = await scriptRequest('/api/v1/scripts?health=1');
    scriptSystemAvailable = Boolean(data?.script_system?.available);
    scriptSystemMessage = scriptSystemAvailable
      ? ''
      : 'The production database does not currently contain the Script Manager tables. The existing Worker and D1 binding are preserved; no local or fake storage is used.';
  } catch (error) {
    scriptSystemAvailable = false;
    scriptSystemMessage = error.message || 'Unable to inspect Script Manager database state.';
  }
  return scriptSystemAvailable;
}

function scriptSystemBanner() {
  return scriptSystemAvailable ? '' : `<div class="script-schema-warning"><b>Script Manager is waiting for its existing database schema.</b><span>${html(scriptSystemMessage)}</span></div>`;
}
...
function scriptPage() {
  ...
  <section class="panel"><div class="scripts-toolbar"><div class="field"><label for="script-search">Search</label><input id="script-search" placeholder="Name, product or description" ${scriptSystemAvailable?'':'disabled'}></div><div class="field"><label for="script-status">Status</label><select id="script-status" ${scriptSystemAvailable?'':'disabled'}><option value="">All statuses</option><option>ACTIVE</option><option>DISABLED</option></select></div><button class="primary" id="script-refresh">Refresh</button><button class="primary" id="script-create" ${scriptSystemAvailable?'':'disabled'}>+ New Script</button></div></section>
  ${scriptSystemBanner()}
  ...
  document.querySelector('#script-refresh').onclick = async()=>{ await checkScriptSystem(); scriptPage(); };
...
function renderScripts() {
  ...
  if (!scriptItems.length) { grid.innerHTML = scriptSystemAvailable ? '<div class="empty"><b>No scripts found</b><span>Create a script and associate it with an active product.</span></div>' : '<div class="empty"><b>Script data is unavailable</b><span>The UI remains ready, but the existing production Script Manager schema is not present.</span></div>'; return; }
...
async function createScriptModal(){
  if (!(await checkScriptSystem())) { alert(scriptSystemMessage); return; }
  ...
  if (!products.length) { alert('No active products are available for a new script.'); return; }
